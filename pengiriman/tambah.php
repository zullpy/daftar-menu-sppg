...
// ═══════════════════════════════════════════════════════════
// KURANGI STOK GUDANG PUSAT (tabel `barang`)
// ═══════════════════════════════════════════════════════════
function kurangiStokGudangPusat($pdo_draft, $nama_barang, $qty)
{
    $stmt = $pdo_draft->prepare("SELECT id_barang, stok_akhir FROM barang WHERE nama_barang = ?");
    $stmt->execute([$nama_barang]);
    $row = $stmt->fetch();

    if ($row) {
        $new_stok = (int)$row['stok_akhir'] - (int)$qty;
...
function kembalikanStokGudangPusat($pdo_draft, $nama_barang, $qty)
{
    $stmt = $pdo_draft->prepare("SELECT id_barang, stok_akhir FROM barang WHERE nama_barang = ?");
    $stmt->execute([$nama_barang]);
    $row = $stmt->fetch();
    ...
    try {
        ...
        if ($edit_id) {
            ...
            foreach ($old_details->fetchAll() as $old) {
                kembalikanStokGudangPusat($pdo_draft, $old['nama_barang'], $old['qty']);
            }
            ...
            $stmt->execute([
                $no_surat_jalan, $nama_sppg, $nama_pengirim, $alamat,
                $tanggal_ekspedisi, $ekspedisi, $lokasi, $edit_id
            ]);
        ...
        for ($i = 0; $i < count($nama_barangs); $i++) {
            ...
            if (!empty($nama_b)) {
                ...
                kurangiStokGudangPusat($pdo_draft, $nama_b, $qty_b);
            }
        }
    ...
    <main>
        ...
        <div class="info-card" style="border-left-color: var(--amber);">
            ...
            <p>Ketika Anda mengirim barang ke gudang transit (Sodong/Sariwangi/Manonjaya), 
               ...
               Ketik nama barang untuk melihat stok tersedia.</p>
        </div>
